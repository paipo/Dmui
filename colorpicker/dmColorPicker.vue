  ...
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    ...
    <div class="box" v-bind:class="{ open: openStatus }">
      ...
      <div class="bd">
        ...
        <ul class="bColor">
          <li v-for="item of colorPanel" v-bind:key="item.id">
            <ul>
              <li
                v-for="color of item"
                v-bind:key="color.id"
...
.m-colorPicker{
  ...
  input{ display: none; }
  .colorBtn{ width: 15px; height: 15px; -webkit-box-shadow: 0px 0px 2px #ccc; cursor: pointer; }
  .colorBtn.disabled{ cursor: no-drop; }
  .box{
    position: absolute; z-index: 900; margin-left: -86px; width: 220px; background: #fff; border: 1px solid #ddd; visibility: hidden; border-radius: 2px; margin-top: 2px; padding: 10px; padding-bottom: 5px; box-shadow: 0 0 5px rgba(0,0,0,.15); opacity: 0; transition: all .3s ease;
  ...
  .hd{
    ...
    .colorView{ width: 100px; height: 30px; float: left; transition: background-color .3s ease;-webkit-box-shadow: 0px 0px 2px #ccc; }
    .defaultColor{ width: 70px; float: right; text-align: center; border: 1px solid #ddd; cursor: pointer; margin-right: 10px }
  }
  .tColor{
